...
module {
  tfg.func @mlir_lifted_graph(%arg: tensor<2x2xf32> {tfg.name = "name"}) {
    ...
    // CHECK: %[[ACC3C:.*]], {{.*}} = Const{{.*}} name("acc3/_partial_split_2")
    ...
    // CHECK: %[[ACC4C:.*]], {{.*}} = Const{{.*}} name("acc4/_partial_split_2")
    ...
    // CHECK: %[[ACC5C:.*]], {{.*}} = Const{{.*}} name("acc5/_partial_split_2")
    ...
    // CHECK: %[[ACC6C:.*]], {{.*}} = Const{{.*}} name("acc6/_partial_split_2")
