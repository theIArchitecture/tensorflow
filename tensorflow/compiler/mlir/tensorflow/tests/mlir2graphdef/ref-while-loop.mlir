...
func.func @main() {
  ...
  tf_executor.graph {
    ...
    %1:2 = tf_executor.island wraps "tf.VariableV2"() {device = "", dtype = "tfdtype$DT_INT32", value = dense<0> : tensor<i32>, shape = #tf_type.shape<0>, container = "", shared_name = ""} : () -> tensor<!tf_type.int32ref> loc("Ref_Variable")
    %2:2 = tf_executor.Enter %1#0 frame "while/while_context" parallel_iterations 10 : (tensor<!tf_type.int32ref>) -> (tensor<*x!tf_type.int32ref>, !tf_executor.control) {device = "", T = "tfdtype$DT_INT32"} loc("while/Enter")
    ...
    %7:3 = tf_executor.Switch %3#0, %6#0 : (tensor<*x!tf_type.int32ref>, tensor<i1>) -> (tensor<*x!tf_type.int32ref>, tensor<*x!tf_type.int32ref>, !tf_executor.control) {device = "", T = "tfdtype$DT_INT32", _class = ["loc:@while/Merge"]} loc("while/Switch")
